...
//按键的高宽比
private let KEYRATIO:CGFloat = 12.0  / 63.0

//键盘的高
private let KEYBOARD_HEIGHT:CGFloat = KSCREEN_WIDTH * BOARDRATIO

//按键的宽
private let BTN_WIDTH:CGFloat = KSCREEN_WIDTH / 3.0 - 6.0
...
//总高
private let TOTAL_HEIGHT:CGFloat = ITEM_HEIGHT * 5 + 10.0 + SAFE_BOTTOM
...
class KeyBoardView: UIView {
    ...
    lazy private var topView:UICollectionView = {
        ...
        //设置UICollectionView
        let collectionView = UICollectionView.init(frame: CGRect.init(x: 0, y: 0, width: KSCREEN_WIDTH / 3.0 * 2.0, height: ITEM_HEIGHT), collectionViewLayout: flowLayout)
    ...
    lazy private var middleView:UICollectionView = {
        ...
        //设置UICollectionView
        let collectionView = UICollectionView.init(frame: CGRect.init(x: 0, y: ITEM_HEIGHT , width:  KSCREEN_WIDTH , height: ITEM_HEIGHT * 3), collectionViewLayout: flowLayout)
        collectionView.delegate = self
        collectionView.dataSource = self
        collectionView.backgroundColor = UIColor.clear
        collectionView.register(KeyBoardCell.self, forCellWithReuseIdentifier: CELL_ID)
    ...
    lazy private var bottomView:UICollectionView = {
        ...
        //设置UICollectionView
        let collectionView = UICollectionView.init(frame: CGRect.init(x: 0, y: ITEM_HEIGHT * 4, width:  KSCREEN_WIDTH / 3.0 * 2.0, height: ITEM_HEIGHT), collectionViewLayout: flowLayout)
        collectionView.delegate = self
        collectionView.dataSource = self
        collectionView.backgroundColor = UIColor.clear
        collectionView.register(KeyBoardCell.self, forCellWithReuseIdentifier: CELL_ID)
    ...
    lazy private var clearBtn:UIButton = {
        ...
        //设置位置
        btn.frame = CGRect.init(x: KSCREEN_WIDTH - 3 - BTN_WIDTH, y: TOTAL_HEIGHT - BTN_HEIGHT - 10 - SAFE_BOTTOM, width: BTN_WIDTH, height: BTN_HEIGHT)
    ...
    lazy private var doneBtn:UIButton = {
        ...
        //设置位置
        btn.frame = CGRect.init(x: KSCREEN_WIDTH - 3 - BTN_HEIGHT, y: TOTAL_HEIGHT - BTN_HEIGHT - 10 - BTN_HEIGHT - 10 - BTN_HEIGHT - 10 - BTN_HEIGHT - 10 - BTN_HEIGHT - 10 - SAFE_BOTTOM , width: BTN_HEIGHT , height: BTN_HEIGHT)
...
extension KeyBoardView{
    ...
    private func setData(){
        ...
        //添加小数点.
        self.modelArray.append(KeyBoardModel.init(key: ".", isUpper: self.isUp))
...
extension KeyBoardView : UICollectionViewDelegate,UICollectionViewDataSource{
   ...
   public func numberOfSections(in collectionView: UICollectionView) -> Int{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        var index = indexPath.section * 3 + indexPath.item
        //获取index
        if (collectionView == self.topView){
            return cell
        }
        ...
        if (collectionView == self.middleView){
            ...
            index = 9 + indexPath.section * 3 + indexPath.item
        }
